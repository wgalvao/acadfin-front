// funcionarios => page.js
"use client";
import { useRouter, useParams } from "next/navigation"; // Importa o hook useRouter
...
const Funcionarios = () => {
  ...
  const { id } = useParams(); // Captura o ID da URL, se houver
  const router = useRouter();
  ...
  useEffect(() => {
    if (id) {
      fetch(`http://localhost:8000/api/funcionarios/${id}`)
        .then((res) => res.json())
  ...
  const handleSubmit = async (e) => {
    ...
    if (!result.success) {
      ...
      try {
        const formattedData = {
          ...formData,
          data_nasc: formData.data_nasc.split("/").reverse().join("-"),
        };
            ...
            <Form onSubmit={handleSubmit}>
              ...
              <div className="d-grid gap-2">
                <Button
                  ...
                  type="submit"
                >
                  <Save /> {id ? "Atualizar" : "Salvar"} informações
                </Button>
              </div>
            </Form>
          </div>

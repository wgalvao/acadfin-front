...
const Home = () => {
  ...
  useEffect(() => {
    if (id) {
      fetchCentroCustoById(id)
        .then((data) => {
          setFormData({ ...data });
          setIsCreating(false); // Set to false since we are editing
        })
        .catch((error) =>
          console.error("Erro ao carregar dados do centro de custo:", error)
        );
    }
  }, [id]);
  ...
  const handleSubmit = async (e) => {
    ...
    const result = validationSchemaCentroCusto.safeParse(formData);
    ...
    if (!result.success) {
      ...
      try {
        ...
        let response;
        if (isCreating) {
          // Chama createCentroCusto com apenas formData
          response = await createCentroCusto(formData);
        } else {
          // Chama updateCentroCusto com id e formData
          response = await updateCentroCusto(id, formData);
        }
        ...
        if (response) {
          ...
          router.push("/pages/centro-custo");
        } else {
